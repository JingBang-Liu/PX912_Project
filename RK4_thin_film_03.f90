...
MODULE finite_difference_operators
  ...
  ! calculate f
  FUNCTION fun(dx,h1,h2,h3,h4,h5,Ca)
    REAL(KIND=dbl), INTENT(IN) :: dx, h1, h2, h3, h4, h5, Ca ! input
    REAL(KIND=dbl) :: fun ! output

    fun = 1.0_dbl/3.0_dbl/Ca/2.0_dbl*()
  END FUNCTION fun
...
!  PRINT*, x
  ...
  do while ((k<m).and.(test==0))
    ...
    do i=3,n+2 ! calculate k1
      k1(i) = dt*fun(dx,h(k-1,i-2),h(k-1,i-1),h(k-1,i),h(k-1,i+1),h(k-1,i+2),Ca)
    end do
    k1(1) = k1(5)
    k1(2) = k1(4)
    k1(n+4) = k1(n)
    ...
    do i=3,n+2 ! calculate k2
      k2(i) = dt*fun(dx,h(k-1,i-2)+k1(i-2)/2.0_dbl,h(k-1,i-1)+k1(i-1)/2.0_dbl, &
                 h(k-1,i)+k1(i)/2.0_dbl,h(k-1,i+1)+k1(i+1)/2.0_dbl,h(k-1,i+2)+k1(i+2)/2.0_dbl,Ca)
    end do
    k2(1) = k2(5)
    k2(2) = k2(4)
    k2(n+4) = k2(n)
    ...
    do i=3,n+2 ! calcualte k3
      k3(i) = dt*fun(dx,h(k-1,i-2)+k2(i-2)/2.0_dbl,h(k-1,i-1)+k2(i-1)/2.0_dbl, &
                 h(k-1,i)+k2(i)/2.0_dbl,h(k-1,i+1)+k2(i+1)/2.0_dbl,h(k-1,i+2)+k2(i+2)/2.0_dbl,Ca)
    end do
    k3(1) = k3(5)
    k3(2) = k3(4)
    k3(n+4) = k3(n)
    ...
    do i=3,n+2 ! calculate k4
      k4(i) = dt*fun(dx,h(k-1,i-2)+k3(i-2),h(k-1,i-1)+k3(i-1), &
                 h(k-1,i)+k3(i),h(k-1,i+1)+k3(i+1),h(k-1,i+2)+k3(i+2),Ca)
    end do
    k4(1) = k4(5)
    k4(2) = k4(4)
    k4(n+4) = k4(n)
